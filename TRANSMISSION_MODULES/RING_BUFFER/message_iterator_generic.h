...
template<HEADER_GENERIC H> int MessageIteratorGenericBase<H>::MessageSize(uint8_t* PtrMessage)
{
	auto& Header = GetHeader(PtrMessage); 
	  if(!Header.isValid()) return 0; 
						    return Header.getMessageSize();
}
template<HEADER_GENERIC H> int MessageIteratorGenericBase<H>::MessageSize()      
{ 
...
template<HEADER_GENERIC H, IteratorMode Mode=IteratorMode::Chunked>
