...
public:
  ...
  std::shared_ptr<PortAdapter<DeviceRotaryGenericInterface>> PortMoveRelative = nullptr;
	...
	void linkTo(WidgetRotaryPlatformControl* Window ) { Window->linkToDevice(this); ControlWindows.push_back(Window);}
  void notifyControlWindow();
...
template<typename T_CONNECTION, typename T_COMMAND, typename T_MESSAGE>
void DeviceRotaryInterface<T_CONNECTION,T_COMMAND,T_MESSAGE>::notifyControlWindow()
{
  if(ControlWindows.empty()) return;
  for(auto window: ControlWindows)
           window->slotDevicePosChanged(Position.first, Position.second);
}
